"""Ablation: does adding the VIX term-structure shape (vix_term_slope,
vix_term_slope_back) to module A's direction features actually improve
out-of-sample calibration, or is it noise?

Same discipline as backtest_feature_ablation.py (CLAUDE.md section 12): a
candidate only "passes" via a paired bootstrap CI on its OOS per-sample Brier
delta, never an eyeballed single number. Two things make this a fair, paired
test:

  * ONE feature matrix is built per symbol requiring baseline + VIX columns all
    non-null, so both arms score the exact same rows/trade_dates.
  * The regime track always observes REGIME_COLUMNS in both arms (VIX features
...
Unlike the per-underlying options candidates (gex/skew/put_call), which need
historical option chains yfinance doesn't provide, the VIX term structure has
full free daily history -- so this candidate is genuinely backtestable.

Pooled across several liquid names (not SOXL alone) so the bootstrap has real
power and the result isn't a single-symbol fluke.

Usage:
    .venv/bin/python -m stockmoney.models.backtest_vix_term_ablation --db data/stockmoney_live.duckdb
"""
...
VIX_COLUMNS = ["vix_term_slope", "vix_term_slope_back"]
EXTENDED_COLUMNS = BASELINE_COLUMNS + VIX_COLUMNS
...
def main(db_path: str = DEFAULT_DB_PATH) -> None:
    ...
    print(f"Candidate +VIX: {VIX_COLUMNS}\n")
    ...
    for symbol, sector in SYMBOLS:
        ...
        print(f"{symbol:5} n={bo.n:4}  "
              ...
              f"+VIX: acc={eo.accuracy:.3f} brier={eo.brier:.3f}  "
    ...
    print(f"\n=== POOLED (n={len(b_all)}) Brier(+VIX) - Brier(baseline) ===")
    print(f"mean diff: {mean_diff:+.4f}  95% CI: [{lo:+.4f}, {hi:+.4f}]")
    if hi < 0:
        print("-> VIX term structure SIGNIFICANTLY improves OOS Brier. Promote to FEATURE_COLUMNS.")
    elif lo > 0:
        print("-> VIX term structure SIGNIFICANTLY worsens OOS Brier. Keep in feature_store, do not promote.")
    else:
